...
import net.caaguazu.turismo.core.Sistema
...
fun PantallaDiagnostico(alVolver: () -> Unit, modifier: Modifier = Modifier) {
    ...
    val lineas = remember(recarga) {
        RegistroVisible.leer().lines().filter { it.isNotBlank() }.asReversed()
    }
    ...
    Column(modifier.fillMaxSize().background(Tono.fondo)) {
        ...
        Column(Modifier.padding(Medida.margen)) {
            Dato(Textos.t("diag.version"), Entorno.version)
        ...
        Row(
            ...
            PildoraSuave(Textos.t("diag.compartir"), alTocar = ::compartir)
            PildoraSuave(
...
/**
 * Saca el registro del telefono por donde la persona quiera: correo, WhatsApp,
 * archivos. Es la unica forma que tengo de ver que paso en un telefono que no
 * es mio.
 *
 * Se recortan los ultimos 120 KB y no el archivo entero: mas que eso no entra
 * en un correo y tampoco aporta — lo que interesa es lo ultimo que paso antes
 * del fallo.
 */
private fun compartir() {
    Sistema.compartirTexto("registro-turismo.txt", RegistroVisible.leer().takeLast(120_000))
}
